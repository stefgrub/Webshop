...
(function () {
  ...
  function start() {
    ...
    // Neu messen bei echten Resize-Ereignissen
    let resizeTimer;
    ...
    // ---- Animation & Steuerung ----
    const T = 20;          // Dauer einer vollen Bewegung (s)
    const jitterPx = 0.25; // kleine Wellen
    ...
    function tick(now) {
      ...
      if (!paused) {
        const p = (elapsed % T) / T;
        const y = ease(p) * maxY;
        const j = Math.sin(elapsed * 3.173) * jitterPx + Math.sin(elapsed * 5.411) * (jitterPx * 0.5);
